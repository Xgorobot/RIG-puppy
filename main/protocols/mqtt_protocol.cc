...
bool MqttProtocol::StartMqttClient(bool report_error) {
    ...
    int keepalive_interval = settings.GetInt("keepalive", 30);
    ...
    mqtt_->OnDisconnected([this]() {
        ...
        ESP_LOGW(TAG, "MQTT disconnected, schedule reconnect in %lu ms", reconnect_interval_ms_);
        esp_timer_start_once(reconnect_timer_, (uint64_t)reconnect_interval_ms_ * 1000);
        // 指数退避：3s -> 6s -> 12s -> 24s -> 30s
        reconnect_interval_ms_ = std::min(reconnect_interval_ms_ * 2, (uint32_t)MQTT_RECONNECT_MAX_MS);
    });
    ...
    mqtt_->OnConnected([this]() {
        ...
        esp_timer_stop(reconnect_timer_);
        reconnect_interval_ms_ = MQTT_RECONNECT_INITIAL_MS;  // 连接成功，重置退避计数
        ESP_LOGI(TAG, "MQTT connected, reconnect interval reset");
    });
...
std::string MqttProtocol::GetHelloMessage() {
    ...
    if (Application::GetInstance().GetAecMode() == kAecOnServerSide) {
