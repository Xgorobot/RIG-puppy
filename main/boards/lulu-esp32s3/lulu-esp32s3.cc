...
class LuluEsp32s3Board : public WifiBoard {
private:
    Button boot_button_;
    ...
    void InitializeButtons() {
        ...
        esp_timer_create_args_t timer_args = {
            .callback = [](void* arg) {
                auto board = static_cast<LuluEsp32s3Board*>(arg);
                // 检查按键是否仍被按住
...
public:
    LuluEsp32s3Board() : boot_button_(BOOT_BUTTON_GPIO) {
        InitializeSpi();
...
DECLARE_BOARD(LuluEsp32s3Board);
