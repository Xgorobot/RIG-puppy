...
PowerSaveLevel Application::GetUserPowerSaveLevel() {
    ...
    std::string mode = settings.GetString("power_mode", "low_power");
    if (mode == "performance") {
        return PowerSaveLevel::PERFORMANCE;
    } else if (mode == "balanced") {
        return PowerSaveLevel::BALANCED;
    }
    return PowerSaveLevel::LOW_POWER;
}
...
Application::Application() {
    ...
    // AEC 模式：优先从 NVS 读取用户设置，否则使用编译时默认值
    {
        AecMode compile_default = kAecOff;
#if CONFIG_USE_DEVICE_AEC && CONFIG_USE_SERVER_AEC
#error "CONFIG_USE_DEVICE_AEC and CONFIG_USE_SERVER_AEC cannot be enabled at the same time"
#elif CONFIG_USE_DEVICE_AEC
        compile_default = kAecOnDeviceSide;
#elif CONFIG_USE_SERVER_AEC
        compile_default = kAecOnServerSide;
#endif
        Settings settings("audio", false);
        int saved_mode = settings.GetInt("aec_mode", -1);
        if (saved_mode >= kAecOff && saved_mode <= kAecOnServerSide) {
            aec_mode_ = static_cast<AecMode>(saved_mode);
        } else {
            aec_mode_ = compile_default;
        }
        ESP_LOGI(TAG, "AEC mode: %d (0=off, 1=device, 2=server)", aec_mode_);
    }
...
void Application::Initialize() {
    ...
    // 提前加载 assets 资源，确保后续图片和语音显示正常
    auto& assets = Assets::GetInstance();
    if (assets.partition_valid()) {
        assets.Apply();
        ESP_LOGI(TAG, "Assets loaded during startup");
    }
    ...
    // 检查舵机标定状态（在资源加载后、WiFi连接前）
    board.CheckCalibration(display, audio_service_);
...
void Application::HandleStateChangedEvent() {
    ...
    switch (new_state) {
        ...
        case kDeviceStateListening:
            ...
            // Set emotion AFTER enabling voice processing to avoid EAF loading delay
...
void Application::SetAecMode(AecMode mode) {
    ...
    // 持久化到 NVS
    {
        ...
        settings.SetInt("aec_mode", static_cast<int>(mode));
    }
...
void Application::ResetProtocolSync() {
    // Synchronous version, execute directly in current thread
    if (protocol_ && protocol_->IsAudioChannelOpened()) {
        protocol_->CloseAudioChannel();
    }
    protocol_.reset();
